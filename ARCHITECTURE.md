...
5. **(v14.2–v14.12, feature-flag gated per domain — `OFFLINE_TASKS` for
   ...
   *proposes* only (never writes) and stores the proposal via
   ...
   above. **v14.9–v14.10 add the Habit domain** (`OFFLINE_HABITS`):
   ...
   `EDIT_TASK` — Tier 0 groups it there despite being read-only) —
   plus two deterministic writes (v14.10): habit creation
   (`addhabit`/`add habit`/`new habit <desc>`, `ADD_TASK` — **direct
   apply, no confirm**, matching Legacy's real `addhabit_cmd()`; only
   the AI HABIT flow confirms) and skip (`skiphabit`/`skip habit`/
   `reset streak <id>`, `EDIT_TASK` — direct apply, the reset is
   self-healing). Habit "update" and "delete" need no habit code:
   habits are task rows, so the edit and delete flows above already
   cover them. **v14.11 completed the domain**: `done <id>` on a habit
   now runs `complete_habit` (streak log + reply, NO learning logs —
   Legacy's exact habit branch) through the same completion spec the
   Task domain registers — one handler for both domains, branching on
   `is_habit()` after a single fetch, exactly like Legacy's
   `done_task()`. The habit handler is injected at registry
   construction only when both domains are enabled (ADR-013); a
   tasks-only build still branches habits away to Legacy. Any match here means steps 6-10 below never run for that
...
| `fmt.py` | Telegram-HTML formatting helpers (`esc`, `b`, `i`, `code`, `task_line`, `confirm_box`, blockquotes/spoiler/`code_block`/`link`) | All user content passes through `esc()` |
| `ui.py` | Dashboard card renderers (7 card types) | See [docs/dashboard.md](docs/dashboard.md); Phase 1 of the UI overhaul re-expresses these on `ui_components.py` |
| `ui_components.py` | UI Phase 0 component library — page skeleton, cards, states, canonical empty states, button/keyboard builders, closed icon/label vocabularies, mechanical spec enforcement | Implements [UI_SPEC_v1.md](UI_SPEC_v1.md) §12; deliberately unwired until Phase 1+ (Shadow-Mode discipline) |
| `analytics/` | Restored in v15.7 (Phase 5B.1): `usage_logger.py` (zero-latency background logging), `usage_service.py` (dashboard queries), `model_metrics.py` (rollups & health), `token_counter.py` (pricing & token estimations), `performance_tracker.py` (latency percentiles p50/p95/p99) | Full NVIDIA NIM support, self-test & integration test coverage |
| `run.sh` | Crash-loop restarter (`while true; python3 main.py; sleep 5`) | Assumes `~/telegram-planner-bot` and canonical `venv/`. Safe to run redundantly since v13.1 — a second copy just gets blocked by `instance_lock.py` and retries harmlessly, becoming an automatic standby |
...
**Added in v14.22 (`core/selftest/`):** Admin-only runtime health runner reachable from the Debug Menu's 🧪 Self Test button — registration-based so every future feature registers its own live checks without editing the runner (see [docs/selftest.md](docs/selftest.md)). This also delivered the first piece of UI_SPEC §10's Developer Center (the admin-only `/debug` menu).
