...
**Since v14.6 (Offline Engine write operations 1-4, feature-flag gated — OFF today):**
...
5. **(v14.2–v14.6, feature-flag gated — `OFFLINE_TASKS`, OFF today)** If
   ...
   `Intent.UNKNOWN`, first a completion phrase (`done <id>`/`complete
   task <id>`/`finish task <id>`/`mark done <id>` — v14.6), then an
   ...
   *proposes* only (never writes) and stores the proposal via
   ...
   message. On no match (e.g. `/habits`, "remind me to...", bare "done"),
