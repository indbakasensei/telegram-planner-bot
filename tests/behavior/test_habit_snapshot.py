"""
Snapshot regression lock tests for Habit behavior — Phase 4B.

These tests lock in the CURRENT characterized behavior from Phase 4A
as immutable baselines. They must never be modified to "improve" the
behavior — only to document a deliberate, reviewed change to the
regression baseline itself.

Run: pytest tests/behavior/test_habit_snapshot.py -v
"""
import sqlite3
...
from core.actions import create_habit, complete_habit, skip_habit, habit_views, delete_task
...
def test_snapshot_habit_create_basic(storage):
    """Lock in: creating a basic habit stores it with expected defaults."""
    ...
    # === SNAPSHOT BASELINE: these must match exactly ===
    assert row[2] == "Meditate"           # title (index 2)
...
def test_snapshot_habit_create_with_time(storage):
    """Lock in: creating a habit with time stores due_time."""
...
def test_snapshot_habit_create_weekly(storage):
    """Lock in: creating a weekly habit stores recurrence_type and weekday."""
...
def test_snapshot_habit_create_monthly(storage):
    """Lock in: current behavior — monthly not recognized, returns daily."""
...
def test_snapshot_habit_create_custom_category(storage):
    """Lock in: creating a habit with custom category stores it."""
...
def test_snapshot_habit_complete_first_time(storage):
    """Lock in: completing a habit for the first time logs completion and sets streak to 1."""
    ...
    # === SNAPSHOT: completed column at index 4 ===
    assert row[4] == 1  # completed = 1


def test_snapshot_habit_complete_increments_streak(storage):
    """Lock in: completing a habit on consecutive days increments streak."""
    ...
    conn.close()
    # === SNAPSHOT: streak columns ===
    assert row[0] == 2  # current_streak
    assert row[1] == 2  # longest_streak


def test_snapshot_habit_complete_already_done_today(storage):
    """Lock in: completing a habit already logged today succeeds with zero writes."""
...
def test_snapshot_habit_complete_broken_streak_resets(storage):
    """Lock in: completing after a gap resets streak to 1."""
...
def test_snapshot_habit_complete_paused_habit(storage):
    """Lock in: completing a paused habit works (paused flag doesn't block completion)."""
    hid = _seed_habit(storage, "Meditate")
...
def test_snapshot_habit_complete_nonexistent(storage):
    """Lock in: completing a nonexistent habit fails."""
    result = complete_habit.execute_by_id(99999, UID, storage)
    assert result.success is False
    assert "task_not_found" in result.warnings


def test_snapshot_habit_complete_not_a_habit(storage):
    """Lock in: completing a regular task via habit completion fails."""
...
def test_snapshot_habit_skip_resets_streak(storage):
    """Lock in: skipping a habit resets current_streak to 0."""
    hid = _seed_habit(storage, "Meditate", days_logged=(1, 0))  # streak 2
    result = skip_habit.execute(hid, ctx_edit(f"skip habit {hid}"), storage)
    assert result.success is True
...
def test_snapshot_habit_skip_zero_streak(storage):
    """Lock in: skipping a habit with zero streak works."""
    hid = _seed_habit(storage, "Meditate")
    result = skip_habit.execute(hid, ctx_edit(f"skip habit {hid}"), storage)
    assert result.success is True
...
def test_snapshot_habit_skip_nonexistent(storage):
    """Lock in: skipping a nonexistent habit fails."""
    result = skip_habit.execute(99999, ctx_edit("skip habit 99999"), storage)
    assert result.success is False
    assert "not_a_habit" in result.warnings


def test_snapshot_habit_skip_not_a_habit(storage):
    """Lock in: skipping a regular task fails."""
    tid = storage.tasks.add(UID, "Plain task")
    result = skip_habit.execute(tid, ctx_edit(f"skip habit {tid}"), storage)
    assert result.success is False
    assert "not_a_habit" in result.warnings


# ── STREAK INCREMENT SNAPSHOTS ───────────────────────────────────────────


def test_snapshot_habit_streak_increment_daily_consecutive(storage):
    """Lock in: daily habit: consecutive completions increment streak."""
    hid = _seed_habit(storage, "Meditate")
...
def test_snapshot_habit_streak_increment_weekly(storage):
    """Lock in: weekly habit: completions on correct weekdays increment streak."""
...
def test_snapshot_habit_longest_streak_preserved(storage):
    """Lock in: longest streak is preserved when current streak drops."""
...
def test_snapshot_habit_delete_removes_task_and_log(storage):
    """Lock in: deleting a habit removes task row (but NOT habit_log entries - no cascade in Legacy)."""
    ...
    rows = conn.execute("SELECT * FROM habit_log WHERE habit_id=?", (hid,)).fetchall()
    conn.close()
    # === SNAPSHOT: orphaned habit_log entries ===
    assert len(rows) == 2  # habit_log entries remain (no cascade)


def test_snapshot_habit_delete_not_found(storage):
    """Lock in: deleting a nonexistent habit succeeds idempotently (already_deleted flag)."""
    from core.actions import delete_task
...
def test_snapshot_habits_list_empty_onboarding(storage):
    """Lock in: empty habits list shows onboarding message."""
...
def test_snapshot_habits_list_renders_streak_fires(storage):
    """Lock in: habits list renders fire emojis for streak."""
...
def test_snapshot_habits_list_caps_fires_at_five(storage):
    """Lock in: habits list caps fire emojis at 5."""
...
def test_snapshot_habits_list_zero_streak_shows_circle(storage):
    """Lock in: habits list shows circle for zero streak."""
...
def test_snapshot_habits_list_excludes_paused(storage):
    """Lock in: habits list excludes paused habits."""
...
def test_snapshot_habit_streak_detail_grid(storage):
    """Lock in: streak detail renders 14-day grid."""
    hid = _seed_habit(storage, "Meditate", days_logged=(2, 1, 0))
    result = habit_views.streak_detail(hid, ctx_edit(f"streak {hid}"), storage)
    assert result.success is True
...
def test_snapshot_habit_streak_detail_missed_days_warning(storage):
    """Lock in: streak detail shows missed days warning when applicable."""
...
def test_snapshot_habit_log_view_empty(storage):
    """Lock in: habit log view for habit with no entries."""
    hid = _seed_habit(storage, "Meditate")
    result = habit_views.habit_log_view(hid, ctx_edit(f"habitlog {hid}"), storage)
    assert result.success is True
    assert result.data == []
    assert "No log entries yet for <b>Meditate</b>." in result.message


def test_snapshot_habit_log_view_entries_newest_first(storage):
    """Lock in: habit log view shows entries newest first."""
    hid = _seed_habit(storage, "Meditate", days_logged=(1, 0))
    result = habit_views.habit_log_view(hid, ctx_edit(f"habitlog {hid}"), storage)
    assert result.success is True
...
def test_snapshot_habit_log_view_not_a_habit(storage):
    """Lock in: habit log view rejects non-habit."""
    tid = storage.tasks.add(UID, "Plain task")
    result = habit_views.habit_log_view(tid, ctx_edit(f"habitlog {tid}"), storage)
    assert result.success is False
    assert "not_a_habit" in result.warnings
    assert result.message == "That's not a habit."


# ── ENGINE DISPATCH SNAPSHOTS ──────────────────────────────────────────


def test_snapshot_engine_dispatches_create_habit(storage, engine):
    """Lock in: engine dispatches create_habit for recognized phrases."""
...
def test_snapshot_engine_dispatches_habits_list(storage, engine):
    """Lock in: engine dispatches habits_list for recognized phrases."""
...
def test_snapshot_engine_dispatches_streak_and_log(storage, engine):
    """Lock in: engine dispatches streak_detail and habit_log_view."""
    hid = _seed_habit(storage, "Meditate", days_logged=(0,))
...
# ── BEHAVIORAL EQUIVALENCE SNAPSHOTS ──────────────────────────────────


def test_snapshot_create_habit_equivalence_legacy_vs_offline(storage):
    """Lock in: Legacy database.add_habit() vs Offline create_habit produce identical rows."""
    # Legacy path
...
def test_snapshot_complete_habit_equivalence_legacy_vs_offline(storage):
    """Lock in: Legacy log_habit_completion vs Offline complete_habit produce identical state."""
    storage = Storage()
...
def test_snapshot_delete_habit_equivalence_legacy_vs_offline(storage):
    """Lock in: Legacy database.delete_task() vs Offline delete_task produce identical state for habits."""
    storage = Storage()
